...
def list_to_str(input_command):
    debug_split = input_command.split()
    debug_unit = debug_split[1:]
    for i in debug_unit:
        for i in command_list[i].params:
            return i
...
def help_command(input_command):
    ...
    if help_unit not in command_list:
        ...
        print('"start" gives the command to start one of the modules. Expected parameters: {}'.format(command_list[help_unit].params))
    elif help_unit == 'power':
        print('"power" gives the command to increase or decrease engine power. Expected parameters: {}'.format(command_list[help_unit].params))
    elif help_unit == 'velocity':
        print('"velocity" gives the command to increase or decrease velocity. Expected parameters: {}'.format(command_list[help_unit].params))
...
while True:
    ...
    if input_command == 'exit':  # сразу проверяем не хочет ли пользователь выйти
        ...
        com_to_str = [k for k in command_list.keys()]
